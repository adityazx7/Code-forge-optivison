...
import { Layers, Filter, Target, DollarSign } from 'lucide-react';
import { getPlotlyLayout, HEATMAP_COLORSCALE } from '../chartConfig';

export default function OptionsChain({ theme }) {
    ...
    return (
        <div>
            <div className="page-header">
                <div className="page-header-icon"><Layers /></div>
                <div className="page-header-text">
                    <h2>Options Chain Analysis</h2>
                    <p>Strike-wise OI and Volume breakdown with Max Pain calculation</p>
                </div>
            </div>
                ...
                {maxPain && (
                    <>
                        <div className="metric-card orange animate-in">
                            ...
                            <div className="metric-sub">Maximum option seller profit point</div>
                        </div>
            ...
            <div className="charts-grid">
                <div className="card animate-in">
                    ...
                    <Plot
                        data={[
                            { x: oiData.map(d => d.strike), y: oiData.map(d => d.oi_CE), type: 'bar', name: 'Call OI', marker: { color: '#059669', opacity: 0.8 } },
                            { x: oiData.map(d => d.strike), y: oiData.map(d => d.oi_PE), type: 'bar', name: 'Put OI', marker: { color: '#dc2626', opacity: 0.8 } },
                        ]}
                ...
                <div className="card animate-in">
                    ...
                    <Plot
                        data={[
                            { x: volumeData.map(d => d.strike), y: volumeData.map(d => d.volume_CE), type: 'bar', name: 'Call Volume', marker: { color: '#2563eb', opacity: 0.8 } },
                            { x: volumeData.map(d => d.strike), y: volumeData.map(d => d.volume_PE), type: 'bar', name: 'Put Volume', marker: { color: '#d97706', opacity: 0.8 } },
                        ]}
                ...
                <div className="card full-width animate-in">
                    <div className="card-header">
                        <div className="card-title"><Target /> OI Heatmap — Strike vs. Date</div>
                    </div>
                    <Plot
                        data={[{
                            z: zValues,
